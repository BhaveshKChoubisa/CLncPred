...
def calculate_orf_features(seq):
    ...
    return (max(orf_lengths) if orf_lengths else 0,
            sum(orf_lengths) / len(seq) * 100 if orf_lengths else 0)

# ------------------- FEATURE EXTRACTION -------------------
def extract_features(sequences):
    selected_tetramers = ['CAAG', 'TAGC', 'TAGG', 'TTCA', 'ATG', 'TCG', 'AAAC', 'TAAG',
                          'GCAG', 'GAGT', 'AGAT', 'GCCG', 'CATA', 'GCTG', 'ACGA', 'CGTG',
                          'GGAG', 'CTTC', 'CTGA']
    selected_codons = ['CGA', 'CGCT', 'ACC', 'GGA', 'CCGG']
    ...
    for id_name, seq in sequences:
        ...
        # Add selected tetramers
        feature_values.update({tet: tetramers.get(tet, 0) for tet in selected_tetramers})

        # Add selected codons
        feature_values.update({cod: codons.get(cod, 0) for cod in selected_codons})
